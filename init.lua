...
function farming.plant_growth_timer(pos, elapsed, node_name)
	...
	if stages.plant_name == "farming:cocoa" then
		if not minetest_find_node_near(pos, 1, { "default:jungletree", "moretrees:jungletree_leaves_green" }) then
			return true
		end
	else
		local dirt_pos = { x = pos.x, y = pos.y - 1, z = pos.z }
		local under = minetest.get_node_or_nil(dirt_pos)
